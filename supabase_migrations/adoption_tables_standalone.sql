-- Standalone Adoption Tables for Pet Portal
-- This file contains adoption tables without external dependencies
...
CREATE TABLE IF NOT EXISTS adoption_pets (
    ...
    shelter_id INTEGER, -- Will link to vet_owner_branches when available
    listed_by UUID, -- Will link to users when available
    listed_date TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    last_updated TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    adoption_date TIMESTAMP WITH TIME ZONE,
    adopted_by UUID, -- Will link to users when available
...
CREATE TABLE IF NOT EXISTS adoption_applications (
    ...
    review_notes TEXT,
    reviewed_by UUID, -- Will link to users when available
...
CREATE TABLE IF NOT EXISTS adoption_favorites (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL, -- Will link to users when available
    pet_id UUID NOT NULL REFERENCES adoption_pets(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS adoption_inquiries (
    ...
    reply_message TEXT,
    replied_by UUID, -- Will link to users when available
...
CREATE TABLE IF NOT EXISTS adoption_events (
    ...
    branch_id INTEGER, -- Will link to vet_owner_branches when available
    created_by UUID NOT NULL, -- Will link to users when available
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TRIGGER update_adoption_pets_updated_at BEFORE UPDATE ON adoption_pets
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_adoption_applications_updated_at BEFORE UPDATE ON adoption_applications
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_adoption_inquiries_updated_at BEFORE UPDATE ON adoption_inquiries
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_adoption_events_updated_at BEFORE UPDATE ON adoption_events
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_adoption_success_stories_updated_at BEFORE UPDATE ON adoption_success_stories
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
-- Note: RLS policies can be added later when users and vet_owner_branches tables exist
-- For now, the tables are accessible without restrictions for development purposes