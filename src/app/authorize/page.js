...
// Supabase setup (you'll need to install @supabase/supabase-js)
...
import waitSec from "@/components/utils/waitSec";
...
const AuthSignUser = () => {
  ...
  // Form states - moved to top to fix hooks order
  ...
  // Get branches data for branch name dropdown
  ...
  // Force initialization after a timeout if stuck
  useEffect(() => {
    const timeout = setTimeout(() => {
      if (!isInitialized) {
        console.warn("Auth initialization taking too long, forcing form display");
  ...
  useEffect(() => {
    if (
      ...
      // Check if browser has history to go back
      if (window.history.length > 1) {
        router.back();
      } else {
        // No history, redirect to homepage
        router.push("/");
      }
    }
  ...
  if ((!isInitialized || authLoading) && !forceShowForm) {
    return (
  ...
  // Don't render if user is authenticated but not vet-owner (redirect is happening)
  if (isAuthenticated && userRole && userRole !== "vet-owner") {
    return (
      <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-orange-100 flex items-center justify-center">
        <div className="text-center">
          <div className="w-12 h-12 border-4 border-orange-600 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
        ...
        { email: signinForm.email, password: signinForm.password },
        {
          onSuccess: (data) => {
            ...
            const userRole =
              ...
              data?.profile?.role ||
              "pet-owner"; // fallback role

            setSuccessMessage(
              "Successfully signed in! Redirecting to dashboard..."
            );

            const { user, session, profile } = data;
            ...
            // More robust routing with error handling
            try {
      ...
      signUpUser(
        {
          ...
          getProfile: profile,
          clinicName: signupForm.branchName,
        },
        {
          onSuccess: async () => {
            setSuccessMessage(
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <select
                      ...
                      className="w-full pl-12 pr-4 py-4 border border-gray-200 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all duration-200 bg-white/80 backdrop-blur-sm text-gray-900"
                    >
                  ...
                  <div className="relative">
                    ...
                    <select
                      ...
                      className={`w-full pl-12 pr-4 py-4 border ${
                        errors.branchName ? "border-red-300" : "border-gray-200"
                      } rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all duration-200 bg-white/80 backdrop-blur-sm text-gray-900`}
                      required
                    >
                      ...
                      {branchesLoading ? (
                        ...
                        branches
                          .filter(
                            (branch) => branch.branch_type === "main-branch"
                          )
                          .map((branch) => (
