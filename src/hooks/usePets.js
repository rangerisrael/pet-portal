...
export const usePets = (user, userRole = 'pet-owner') => {
  ...
  const fetchPets = async () => {
    // For vet-owner, we don't need a specific user ID to view all pets
    if (!user?.id && userRole !== 'vet-owner') {
      console.log('❌ No user ID and not vet-owner, cannot fetch pets');
      setPets([]);
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (!supabase) {
        ...
        const filteredMockPets = userRole === 'vet-owner' ?
          mockPets :
          mockPets.filter(pet => pet.owner_id === user?.id);
        console.log(`🔒 Mock data filtered: ${filteredMockPets.length} pets for ${userRole}`);
      ...
      try {
        const testResult = await supabase.from("pets").select("count", { count: 'exact' });
      ...
      console.log('📊 Database query result:', { data: data?.length, error: error?.message });

      if (error) {
        // Handle specific database errors
        if (error.code === '42P01') {
          console.log('❌ Pets table does not exist, using mock data');
          console.log('💡 To fix: Run the database migration in Supabase SQL Editor');
          console.log('📄 File: supabase_migrations/complete_pet_portal_setup.sql');
          const filteredMockPets = userRole === 'vet-owner' ? mockPets : mockPets.filter(pet => pet.owner_id === user?.id);
          setPets(filteredMockPets);
          setUsingMockData(true);
        ...
        console.log('✅ Successfully loaded real data from database:', data?.length, 'pets');
        ...
        const processedPets = data?.map(pet => {
          ...
          return {
            ...
            owner_name: apiUser?.first_name && apiUser?.last_name ?
                       `${apiUser.first_name} ${apiUser.last_name}`.trim() :
                       apiUser?.email?.split('@')[0] || pet.emergency_contact_name || 'Unknown Owner',
            owner_email: apiUser?.email || pet.emergency_contact_name || 'No email',
            owner_phone: apiUser?.phone || pet.emergency_contact_phone || 'No phone',
  ...
  const createPet = async (petData) => {
    ...
    try {
      const cleanedData = {
        ...petData,
        owner_id: user.id,
        weight_kg: petData.weight_kg === "" ? null : parseFloat(petData.weight_kg),
        age_years: petData.age_years === "" ? null : parseInt(petData.age_years),
        age_months: petData.age_months === "" ? null : parseInt(petData.age_months),
